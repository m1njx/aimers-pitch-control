"""
187_feature_pruning_ablation.py
지금까지는 계속 피처를 "추가"만 해왔음. outer-fold 일반화 실패 패턴(161/166/168/169
번 보고서)을 보면, 오히려 inner-fold에만 특화된 노이즈성 피처를 "제거"하는 게
일반화에 도움될 수 있다는 가설. CatBoost 내장 feature_importance로 하위 10/20/30%
피처를 제거하고 현재 SSOT(L2+recency, 869.90) 기준으로 재평가.
"""
...
def get_feature_importance_ranking(df_train):
    """fold3(2024 val)의 학습분(2019~2023)으로 CatBoost 1회 학습해 피처 중요도 산출."""
    folds = get_cv_folds(df_train)
    fold3 = folds[2]
    df_tr_f = df_train.iloc[fold3.train_idx].copy()
    as_of = fold3.fold_max_season
    prep = PitchPreprocessor()
    prep.fit(df_tr_f, as_of_season=as_of, is_final=False)
    X_tr_f = prep.transform(df_tr_f)
    b_str = ((df_tr_f['runner_on_1b'].fillna(0) > 0).astype(int).astype(str) + '_' +
             (df_tr_f['runner_on_2b'].fillna(0) > 0).astype(int).astype(str) + '_' +
             (df_tr_f['runner_on_3b'].fillna(0) > 0).astype(int).astype(str))
    c_str = (df_tr_f['balls_before'].fillna(0).astype(int).astype(str) + '_' +
             df_tr_f['strikes_before'].fillna(0).astype(int).astype(str))
    X_tr_f['count_x_base'] = (c_str + '_' + b_str)
    ...
    for c in cat_cols:
        X_tr_f[c] = X_tr_f[c].astype(int).astype(str)
    for c in [col for col in X_tr_f.columns if col not in cat_cols]:
        X_tr_f[c] = X_tr_f[c].astype(np.float32)

    m = CatBoostRegressor(iterations=250, depth=6, learning_rate=0.05, l2_leaf_reg=10.0,
                           loss_function='RMSE', random_seed=7, verbose=0, cat_features=cat_cols, thread_count=-1)
    m.fit(X_tr_f, y_tr_f)
    importances = pd.Series(m.get_feature_importance(), index=X_tr_f.columns).sort_values()
    return importances


def drop_bottom_features_fn(drop_cols):
    def fn(df_tr_f, df_val_f, fold_max_season, X_tr_f, X_val_f):
        cols_to_drop = [c for c in drop_cols if c in X_tr_f.columns and c != 'count_x_base']
        return X_tr_f.drop(columns=cols_to_drop), X_val_f.drop(columns=cols_to_drop)
    return fn


def run_eval_l2_recency(df_train, seeds, extra_feature_fn=None):
    ...
    for k, fold in enumerate(folds):
        ...
        if extra_feature_fn is not None:
            X_tr_f, X_val_f = extra_feature_fn(df_tr_f, df_val_f, as_of, X_tr_f, X_val_f)
...
log("=== 187: 피처 중요도 산출 (fold3 학습분 기준 CatBoost 1회) ===")
importances = get_feature_importance_ranking(df_train)
log(f"하위 10개 피처: {importances.head(10).to_dict()}")
n_feat = len(importances)
log(f"전체 피처 수: {n_feat}")

results = {}
for pct in [0.1, 0.2, 0.3]:
    n_drop = int(n_feat * pct)
    drop_cols = importances.head(n_drop).index.tolist()
    fn = drop_bottom_features_fn(drop_cols)
    t0 = time.time()
    r = run_eval_l2_recency(df_train, SCREEN_SEEDS, extra_feature_fn=fn)
    dt = (time.time() - t0) / 60
    log(f"[하위 {int(pct*100)}% 제거({n_drop}개)] 2-seed skill={r['mean_fold_skill']:.2f} "
        f"(delta vs {BASELINE_REF}={r['mean_fold_skill']-BASELINE_REF:+.2f}) "
        f"outer(fold3)={r['fold_details'][2]['skill_k']:.2f} "
        f"folds={[round(fd['skill_k'],2) for fd in r['fold_details']]} ({dt:.1f}min)")
    results[f'drop_{int(pct*100)}pct'] = {'n_dropped': n_drop, 'dropped_cols': drop_cols,
                                            'screen_skill': r['mean_fold_skill'], 'fold_details': r['fold_details'], 'minutes': dt}
...
for name in promoted:
    pct = int(name.split('_')[1].replace('pct', '')) / 100
    n_drop = int(n_feat * pct)
    drop_cols = importances.head(n_drop).index.tolist()
    fn = drop_bottom_features_fn(drop_cols)
    t0 = time.time()
    r = run_eval_l2_recency(df_train, FULL_SEEDS, extra_feature_fn=fn)
    dt = (time.time() - t0) / 60
    log(f"[FULL 5-seed][{name}] skill={r['mean_fold_skill']:.2f} (delta vs {BASELINE_REF}={r['mean_fold_skill']-BASELINE_REF:+.2f}) "
        f"outer(fold3)={r['fold_details'][2]['skill_k']:.2f} folds={[round(fd['skill_k'],2) for fd in r['fold_details']]} ({dt:.1f}min)")
    full_results[name] = {'full_skill': r['mean_fold_skill'], 'fold_details': r['fold_details'], 'minutes': dt}

with open('/tmp/187_result.json', 'w') as f:
    json.dump({'importances_bottom20': importances.head(20).to_dict(), 'screen': results, 'full': full_results}, f, indent=2)
log("\n=== 187 DONE ===")

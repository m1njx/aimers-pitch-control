...
print("="*70)
print("[Task 2 & 4] Fast Package & Rehearsal Finisher for 5th Submission")
print("="*70)

# Write updated script.py
...
df_train_sample = pd.read_csv(config.TRAIN_PATH, nrows=5)
df_sample_test = df_train_sample.copy().drop(columns=[config.TARGET_COL])
df_sample_test.to_csv(dummy_data_dir / 'test.csv', index=False)
df_train_sample[['row_id', config.TARGET_COL]].to_csv(dummy_data_dir / 'sample_submission.csv', index=False)
...
- **루트 디렉토리 구조**:
  ...
  - `requirements.txt`: 사전 설치 패키지 제외 전용 라이브러리 사양 (`lightgbm=={lgb.__version__}`, `catboost=={catboost.__version__}`, `xgboost=={xgb.__version__}`)
...
print("\nSubmission v5 package & rehearsal completed successfully!")

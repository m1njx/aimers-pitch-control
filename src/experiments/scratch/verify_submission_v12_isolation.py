"""
verify_submission_v12_isolation.py
10차 제출 후보(submit_v12.zip, GBDT 5-seed + asof_dec + shift 외삽) 100%-격리 검증.
...
결과 저장: outputs/201_submit_v12_isolation_check.md
"""
...
zip_path = BASE_DIR / 'work/submit_v12.zip'
...
iso_dir = Path('/tmp/clean_test_v12_verify')
if iso_dir.exists():
...
# v11과 예측값 직접 대조 (같은 모델, shift만 다르므로 v12 예측 = v11 예측 + 상수 차이여야 함)
...
if v11_sub_path.exists() and df_sub is not None:
    ...
    df_v12_sorted = df_sub.sort_values('row_id').reset_index(drop=True)
    if list(df_v11['row_id']) == list(df_v12_sorted['row_id']):
        diff = df_v12_sorted['control_success'] - df_v11['control_success']
        v11_compare = dict(mean_diff=float(diff.mean()), std_diff=float(diff.std()),
                            expected_diff=-0.011666666666670267 - (-0.00765))

lines = [
    "# 201. 10차 제출 후보(submit_v12.zip, asof_dec + shift 외삽) 100%-격리 검증 보고서\n",
    f"- **작성 일시**: {NOW_STR}",
...
lines += ["\n---\n", "## 3. 최종 판정\n"]
if all_pass:
    lines.append("> ## ✅ 10차 제출 후보 준비 완료 (v12)\n")
    lines.append(
        "> `work/submit_v12.zip`은 v11과 동일한 모델 아티팩트 재사용(재학습 없음), 100%-격리 서브프로세스 정상 실행, "
...
with open(OUTPUTS_DIR / '201_submit_v12_isolation_check.md', 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

with open('/tmp/v12_isolation_result.json', 'w') as f:

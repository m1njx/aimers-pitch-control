...
# Evaluate on 2023 AND 2024 validation sets to ensure true temporal stability!
...
print(f"2023 Val: {len(df_val23):,} rows | 2024 Val: {len(df_val24):,} rows")
...
print("\n--- RUNNING CONSTRAINED GRID SEARCH ACROSS 2023 AND 2024 ---")

best_score_avg = -1e9
best_params = None

# Grid search realistic, robust parameter ranges:
# w_mlp in [0.35, 0.40, 0.45, 0.50]
# w_mse in [0.15, 0.20, 0.25]
# scale in [1.02, 1.05, 1.08, 1.10, 1.12, 1.15]
# shift in [-0.006, -0.005, -0.0045, -0.0040, -0.0035, -0.0030]

for w_mlp in [0.30, 0.35, 0.40, 0.45, 0.50]:
    for w_mse in [0.15, 0.20, 0.25]:
        ...
        for scale in [1.02, 1.05, 1.08, 1.10, 1.12, 1.15]:
            for shift in [-0.0060, -0.0050, -0.004519, -0.0040, -0.0035, -0.0030]:
                p24_cal = np.clip(0.5 + scale * (p24_cond - 0.5) + shift, 1e-6, 1 - 1e-6)
                p23_cal = np.clip(0.5 + scale * (p23_cond - 0.5) + shift, 1e-6, 1 - 1e-6)
                ...
                if avg_s > best_score_avg:
                    ...
                    best_params = (w_gbdt, w_mlp, w_mse, scale, shift, s24, s23)

w_g, w_m, w_mse, scl, shf, s24, s23 = best_params
print(f"👑 TOP ROBUST CONFIGURATION (2-Year Temporal Holdout Average: {best_score_avg:.2f} pts):")
print(f"  GBDT Binary Weight:  {w_g:.2f} ({w_g*100:.0f}%)")
print(f"  SimpleMLP Weight:    {w_m:.2f} ({w_m*100:.0f}%)")
print(f"  LGBM MSE Weight:     {w_mse:.2f} ({w_mse*100:.0f}%)")
print(f"  Optimal Scale:       {scl:.2f}")
print(f"  Optimal Shift:       {shf:.6f}")
print(f"  2024 Score:          {s24:.2f} pts")
print(f"  2023 Score:          {s23:.2f} pts")

# Compare with v42 exact baseline on 2-year average
...
print(f"\nv42 Baseline 2-Year Average: {v42_avg:.2f} pts | Gain: +{best_score_avg - v42_avg:.2f} pts 🚀")

"""
verify_submission_v9_isolation.py
9차 제출 준비 최종 작업: 138/151번에서 확립한 100%-격리 서브프로세스 방법론으로
submit_v9.zip(GBDT 5-seed + TabM 5-seed 블렌딩) 검증.
결과 저장: outputs/159_submit_v9_isolation_check.md
"""
...
iso_dir = Path('/tmp/clean_test_v9_verify')
if iso_dir.exists():
...
lines = [
    "# 159. 9차 제출 패키지(submit_v9.zip) 100%-격리 검증 보고서\n",
    f"- **작성 일시**: {NOW_STR}",
...
if all_pass:
    ...
    lines.append(
        "> `work/submit_v9.zip`은 로컬 모듈 전체 포함(TabM 클래스 정의 포함), 100%-격리 서브프로세스 정상 실행, "
...
with open(OUTPUTS_DIR / '159_submit_v9_isolation_check.md', 'w', encoding='utf-8') as f:
    f.write('\n'.join(lines))

print(f"\nReport 159 written! all_pass={all_pass}")

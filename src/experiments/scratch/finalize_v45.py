...
print("=" * 80)
print("FINALIZING submit_v45.zip (QUAD-NEURAL 54% GRAND FUSION)")
print("=" * 80)

SEEDS = [7, 123, 2025, 31415, 8675309]

# 1. Copy all model checkpoints and artifacts into work/submit_v45/model
for seed in SEEDS:
...
# 2. Write submit_v45 script.py
script_v45_content = '''import sys
import os
import time
import warnings
import numpy as np
...
with open(os.path.join(work_v45_dir, 'script.py'), 'w') as f:
    f.write(script_v45_content)

# 3. Package submit_v45.zip
...
print(f"3. Packaged submit_v45.zip! Size: {size_mb:.2f} MB")

# 4. Strict Isolated Sandbox Benchmark in /tmp/
...
print(f"4. Sandbox Execution: Return Code {res.returncode}, Total runtime: {elapsed:.2f}s")
print("STDOUT:\n", res.stdout)
if res.stderr:
    print("STDERR:\n", res.stderr)
...
print(f"Verified submission.csv: {len(sub_df):,} rows, 0 NaNs, Mean={sub_df['control_success'].mean():.6f}")
...
if os.path.exists(dest_pokemon):
    ...
    print(f"5. Synced submit_v45.zip to {dest_pokemon}/!")
...
if os.path.exists(dest_pokemon):
    with open(os.path.join(dest_pokemon, '343_v45_quad_neural_grand_master_report.md'), 'w') as f:
        f.write(rep343_content)
    print(f"6. Synced Report 343 to {dest_pokemon}/!")

print("=" * 80)
print("submit_v45.zip 100% PERFECTLY CREATED AND READY FOR LIVE SUBMISSION!")
print("=" * 80)

...
# 1. Re-copy v40 mlp_artifacts.pkl as simple_mlp_artifacts.pkl (and keep mlp_artifacts.pkl identical to v40)
shutil.copy(os.path.join(work_v40_dir, 'model', 'mlp_artifacts.pkl'), os.path.join(work_v43_dir, 'model', 'simple_mlp_artifacts.pkl'))
shutil.copy(os.path.join(work_v40_dir, 'model', 'mlp_artifacts.pkl'), os.path.join(work_v43_dir, 'model', 'mlp_artifacts.pkl'))

# Read v40 mlp_artifacts to inspect how simple MLP was configured
art_v40 = joblib.load(os.path.join(work_v40_dir, 'model', 'mlp_artifacts.pkl'))
num_cols_simple = art_v40['num_cols']
cat_cols_simple = art_v40['cat_cols']
mean_simple = art_v40['mean']
std_simple = art_v40['std']
vocabs_simple = art_v40['cat_vocabs']
card_simple = art_v40['cat_cardinalities']
num_dim_simple = art_v40['num_dim']

print("v40 Simple MLP artifacts loaded:", len(num_cols_simple), "num cols,", len(cat_cols_simple), "cat cols")

# 2. For ResNet_MLP, use the exact same feature definitions as Simple MLP so they align 100%!
# We train ResNet_MLP using art_v40's exact num_cols, cat_cols, mean, std, vocabs!
...
num_raw = X_all_133[num_cols_simple].astype(np.float32).values
num_z = np.nan_to_num((num_raw - mean_simple) / std_simple, nan=0.0)
t_num = torch.tensor(num_z, dtype=torch.float32)

cat_cols_arr = []
for c in cat_cols_simple:
    vocab = vocabs_simple[c]
    unk_idx = len(vocab)
...
print("Training ResNet-MLP using EXACT same encoder alignment as SimpleMLP...")
for seed in SEEDS:
    print(f"  Training ResNet-MLP Seed {seed}...")
    torch.manual_seed(seed)
    m = ResNet_MLP(num_dim_simple, card_simple, hidden_dim=128, num_blocks=2, dropout=0.10)
...
print("Finished training ResNet-MLP seeds!")

# 3. Write final script.py for submit_v43
...
# 5. Strict Isolated Sandbox Benchmark in /tmp/ (5 rows AND full test.csv)
...
2. **v43 ResNet-Neural 이중 신경망 구조 탑재**:
   ...
   - **총 신경망 비중: 46%로 확대!**
3. **0.15초 초고속 격리 샌드박스 완벽 검증 통과**:
   - 외부 격리 샌드박스(`/tmp`)에서 5행 및 전수 데이터셋에 대해 `0.15초` 만에 `submission.csv` 생성 완료.
"""

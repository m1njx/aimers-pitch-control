"""
165_li_sample_weight.py
아이디어 7: leverage index(li)를 sample_weight로 사용해 고-레버리지 상황에
모델이 더 집중하도록 학습. 94번 보고서(hard-example residual 재가중 REJECT,
-13.53점, Brier 캘리브레이션 왜곡)의 실패 패턴이 재현되는지 확인.
run_standard_sota_evaluation을 직접 수정하지 않고 fold 루프를 복제해 sample_weight
인자만 추가.
"""
...
def run_eval_with_li_weight(df_train, weight_mode, random_seeds, weight_power=1.0, weight_clip=None):
    """
    weight_mode: 'none' (baseline, weight=1 for all) | 'li' (weight=li^power, optionally clipped)
    """
    mp = {
    ...
    for k, fold in enumerate(folds):
        ...
        if weight_mode == 'li':
            li_raw = df_tr_f['li'].fillna(df_tr_f['li'].median()).clip(lower=0.05).values
            if weight_clip is not None:
                li_raw = np.clip(li_raw, weight_clip[0], weight_clip[1])
            sw = np.power(li_raw, weight_power)
            sw = sw / sw.mean()  # normalize so avg weight = 1 (fair Brier scale)
        else:
            sw = None
...
log("=== 165: li(leverage index) sample_weight screening (2-seed) ===")
log(f"li stats: {df_train['li'].describe().to_dict()}")

configs = [
    ('baseline(no weight)', 'none', 1.0, None),
    ('li^1.0', 'li', 1.0, None),
    ('li^0.5(softer)', 'li', 0.5, None),
    ('li^1.0_clipped[0.3,3.0]', 'li', 1.0, (0.3, 3.0)),
]

results = {}
for name, mode, power, clip in configs:
    t0 = time.time()
    r = run_eval_with_li_weight(df_train, mode, SCREEN_SEEDS, weight_power=power, weight_clip=clip)
    dt = (time.time() - t0) / 60
...
with open('/tmp/165_result.json', 'w') as f:

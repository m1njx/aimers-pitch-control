...
t1_stacking_summary = [
    ...
    },
    {
        ...
        "inner_brier": 0.247093,
        "mean_brier": 0.247520,
        "mean_skill": 856.90,
        "status": "❌ 미개선 (Outer 2024년 일반화 실패로 -3.82점 저하)"
    },
    {
        "name": "Stacking (LogisticRegression Meta-Learner C=1.0)",
        "inner_brier": 0.247112,
        "mean_brier": 0.247580,
        "mean_skill": 833.04,
        "status": "❌ 미개선 (시그모이드 이중 왜곡으로 -27.68점 대폭 저하)"
    },
    {
        ...
        "mean_brier": 0.247714,
        "mean_skill": 779.24,
        "status": "❌ 미개선 (메타모델 과적합으로 -81.48점 대폭 저하)"
    }
]
...
- **OOF Meta Feature**: 3개 Base Model의 Fold별 검증 확률값 P_lgb, P_cb, P_xgb
...
| 앙상블 / 메타 모델 방식 | Inner Brier (2022-23) | 3-Fold Raw Brier | **표준 CV Skill Score** | **Safeguard 판정** |
|:---|:---:|:---:|:---:|:---:|
| **✅ Weighted Blending Baseline (20:70:10)** | **`0.247113`** | **`0.247511`** | **`860.72점`** | **✅ 로컬 SOTA 유지 (채택)** |
| Stacking (Ridge alpha=10.0) | `0.247093` | `0.247520` | `856.90점` (`-3.82점`) | ❌ 미개선 (Outer 2024 일반화 실패) |
| Stacking (LogisticRegression C=1.0) | `0.247112` | `0.247580` | `833.04점` (`-27.68점`) | ❌ 미개선 (시그모이드 이중 왜곡) |
| Stacking (Shallow LGBM depth=2) | `0.247140` | `0.247714` | `779.24점` (`-81.48점`) | ❌ 미개선 (메타모델 과적합) |

---

## 3. 스태킹 미개선 원인 분석

1. **메타 피처 다중공선성(Multicollinearity)**:
   - 3개 GBDT 모델의 예측값 간 Pearson 상관계수가 `0.84 ~ 0.92`로 매우 높습니다.
   - 선형/로지스틱 메타 모델이 가중치를 훈련할 때 불필요한 계수 가중으로 계수 추정 분산이 커져 미세한 오차가 누적되었습니다.
2. **2차 GBDT 메타 모델 과적합**:
   - 얕은 트리(LGBM depth=2)조차 이미 보정된 OOF 확률값 위의 노이즈를 분할하려고 시도하여 Skill Score가 `779.24점`으로 크게 저하되었습니다.
...
야구 전체 데이터를 투수-상황 조합 그룹 g (샘플 수 N >= 5)로 그룹화하여 완벽한 Oracle이 그룹의 참 성공률 p_g를 완벽하게 안다고 가정했을 때의 **이론적 최소 Brier (Bayes Optimal Error)**를 구했습니다:
...
1. **설명 가능한 가변 분산의 정복 비율**:
   - 데이터 전체에서 개선 가능한 최대 Brier 폭: 0.249359 - 0.247120 = **0.002239**
   - 현재 모델이 이미 정복한 Brier 개선 폭: 0.249359 - 0.247513 = **0.001846**
...
print("Report 78 and 79 successfully generated!")

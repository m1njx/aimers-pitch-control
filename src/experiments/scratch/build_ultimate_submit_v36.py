#!/usr/bin/env python3
"""
build_ultimate_submit_v36.py — Build the Ultimate High-Yield 1050~1100+ Submission Package (submit_v36.zip)

Combines:
1. 133 Features (119 Base + 4 Sabermetric Physics + 10 Domain Interaction Features)
2. 15-Seed GBDT Binary LogLoss (LGB + CB + XGB)
3. 5-Seed LightGBM Direct MSE (Brier Loss)
4. 5-Seed SimpleMLP Direct MSE (nn.MSELoss, 128-64, dropout 0.12)
5. Count-Conditional Micro-Calibration + Affine Post-Processing
6. Local test rehearsal (0.15s, Rule 4 compliant)
7. Final zip packaging to work/submit_v36.zip
"""
...
submit_v36_dir = os.path.join(BASE_DIR, 'work', 'submit_v36')
model_dir = os.path.join(submit_v36_dir, 'model')
...
log("=" * 80)
log("BUILDING ULTIMATE SUBMIT_V36 PACKAGE (1050~1100+ TARGET)")
log("=" * 80)

# Step 1: Copy submit_v33 directory
if os.path.exists(submit_v36_dir):
    shutil.rmtree(submit_v36_dir)
shutil.copytree(submit_v33_dir, submit_v36_dir)
log("Copied submit_v33 to work/submit_v36/")

# Clean unnecessary intermediate files from work/submit_v36/model
...
# Step 2: Load full dataset and build all 133 features
...
log(f"All 133 Features Engineered: {X_all_f.shape[1]} columns")
...
# Step 3: Compute Count-Conditional Shifts
...
unique_counts = np.unique(counts_all)
count_shifts = {}
for cc in unique_counts:
    cc_mask = (counts_all == cc)
    r_cc = y_all[cc_mask].mean()
    # Dampened calibration shift
    count_shifts[cc] = float(r_cc - 0.5) * 0.035
log(f"Computed Count-Conditional Shifts: {count_shifts}")

# Save count_shifts artifact
joblib.dump(count_shifts, os.path.join(model_dir, 'count_shifts_artifact.pkl'))
log("Saved count_shifts_artifact.pkl into submit_v36/model/")

# Step 4: Train 5-Seed LightGBM MSE Models on full 1.47M rows
SEEDS = [7, 123, 2025, 31415, 8675309]
dtr_lgb = lgb.Dataset(X_all_f, label=y_all)

for seed in SEEDS:
    log(f"Training LightGBM MSE Seed {seed} on full data...")
    m_lgb_mse = lgb.train({
...
# Step 5: Train 5-Seed SimpleMLP MSE Models on full 1.47M rows
...
for s_idx, seed in enumerate(SEEDS):
    log(f"Training SimpleMLP_MSE Seed {seed} ({s_idx+1}/5) on full data...")
...
# Step 6: Write complete script.py for submit_v36
...
t0 = time.time()
print("Starting DACON 1100+ Master SOTA Inference Pipeline (v36 Grand Record Ensemble)...")
...
# Multi-Modal Blend: GBDT Binary (45%) + GBDT MSE (20%) + SimpleMLP MSE (35%)
p_raw = W_GBDT_BIN * p_gbdt_bin + W_GBDT_MSE * p_gbdt_mse + W_MLP_MSE * p_mlp_mse
...
with open(os.path.join(submit_v36_dir, 'script.py'), 'w') as f:
    f.write(script_py_content)
log("Updated work/submit_v36/script.py")

# Step 7: Local Test Rehearsal
log("Running local test execution rehearsal on test.csv...")
mkdir_p = os.path.join(submit_v36_dir, 'data')
os.makedirs(mkdir_p, exist_ok=True)
shutil.copy(os.path.join(data_dir, 'test.csv'), os.path.join(mkdir_p, 'test.csv'))

res = subprocess.run([sys.executable, os.path.join(submit_v36_dir, 'script.py')], cwd=submit_v36_dir, capture_output=True, text=True)
print(res.stdout)
if res.returncode != 0:
    print(res.stderr)
...
# Step 8: Package submit_v36.zip
zip_path = os.path.join(BASE_DIR, 'work', 'submit_v36.zip')
if os.path.exists(zip_path):
    os.remove(zip_path)

with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zf:
    for root, dirs, files in os.walk(submit_v36_dir):
        ...
        for file in files:
            ...
            rel_p = os.path.relpath(full_p, submit_v36_dir)
            zf.write(full_p, rel_p)

log(f"Successfully packaged submit_v36.zip! Size: {os.path.getsize(zip_path) / (1024*1024):.2f} MB")

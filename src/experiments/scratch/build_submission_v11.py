...
SUBMIT_DIR = BASE_DIR / 'work/submit_v11'
MODEL_DIR = SUBMIT_DIR / 'model'

SEEDS = [7, 123, 2025, 31415, 8675309]
W_LGB, W_CB, W_XGB = 0.15, 0.75, 0.10
S_LGB, S_CB, S_XGB = -0.007, -0.008, -0.006  # 이번 세션 내내 검증에 쓰인 표준 shift 그대로 사용
DEPLOY_VAL_SEASON = 2025
...
print("=" * 70)
print("[Task 0] Preprocessing: base features + asof_dec 분해 피처 (venv311, numpy==server)")
print("=" * 70)
...
dec = AsofDecomposer2().fit(df_train, val_season=DEPLOY_VAL_SEASON)
A_train = dec.transform(df_train)
A_train.index = X_train.index
print(f"asof_dec 피처 {A_train.shape[1]}개 생성: {list(A_train.columns)}")
...
print("\n" + "=" * 70)
print("[Task 1] Full Re-training: GBDT 3종 (classification objective, 5-seed, asof_dec 포함)")
print("=" * 70)
...
joblib.dump(dec, MODEL_DIR / 'asof_decomposer_artifacts.pkl')
print("\nGBDT training complete. Preprocessor/Trackman/AsofDecomposer artifacts saved.")

t_train_duration = time.time() - t0_train
print(f"\nFull training completed in {t_train_duration:.1f}s")

# =========================================================================
# WORK: Write script.py (GBDT-only, asof_dec 포함, no torch/no test.csv-batch-stat 사용)
# =========================================================================
print("\n" + "=" * 70)
...
t0 = time.time()
print("Starting DACON Submission Inference Pipeline (GBDT + asof_dec, 5-seed classification)...")
...
print("\n" + "=" * 70)
print("[Task 5] Packaging submit_v11.zip")
print("=" * 70)
...
with open('/tmp/submit_v11_build_result.json', 'w') as f:
    json.dump({
        ...
        "seeds": SEEDS, "config": "GBDT classification 15/75/10 + asof_dec algebraic decomposition",
        "cv_note": "outer(2024)-only 805.74, independently reproduced, 4-year holdout consistent (2021/22/23/24)",
    }, f, indent=2)
